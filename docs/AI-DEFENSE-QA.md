...
1. **RecommendationService** — Gợiý sản phẩm cho user dựa trên Collaborative Filtering (Cosine Similarity + KNN) kết hợp Rule-based (best sellers + purchase frequency). Trọng số CF=0.6, RB=0.4.
...
### Q2: Tại sao bạn chọn hybrid approach (CF + Rule-based) mà không dùng单一 approach?

**Trả lời:**
Vì mỗi approach có điểm mạnh riêng:

- **CF** giỏi捕捉 sở thích cá nhân (user tương tự → gợiý tương tự), nhưng yếu với user mới (cold start).
- **Rule-based** giỏi xử lý cold start (best sellers cho user mới) và sản phẩm phổ biến, nhưng không个性化.

Kết hợp cả hai → **"best of both worlds"**: CF đảm bảo个性化, RB đảm bảo cold start. Kết quả ablation study cho thấy Hybrid (F1=0.25) tốt hơn CF-only (0.22) và RB-only (0.18).
...
Kết quả cho thấy F1 cao nhất ở CF=0.6, RB=0.4 (hoặc CF=0.7, RB=0.3 — xấp xỉ). Lý do: CF đóng góp nhiều hơn vì个性化 là yếu tố chính trong recommendation, nhưng RB vẫn cần thiết cho cold start và sản phẩm phổ biến.
...
- K quá lớn (15-20): bao gồm user không相似 → giảm chất lượng.
...
**Trả lời:**
1. Lấy tất cả order items已完成 từ database
2. Với mỗi (userId, productId), cộng dồn quantity
3. Normalize: chia mỗi user vector cho max value → scale 0-1

Sparse matrix:大部分 ô = 0 (user chỉ mua vài sản phẩm trong 50 sản phẩm). Xử lý:
...
1. Classification chỉ cần **argmax** (so sánh tương đối), không cần xác suất绝对 đúng
...
→ Toàn bộ tích = 0 → Bayes theorem崩溃
...
**Trả lời:**
```
Precision = "Trong K gợiý, bao nhiêu % đúng?"
  → Cao = ít gợiý sai, user không bị làm phiền

Recall = "Trong tất cả sản phẩm liên quan, bao nhiêu % được gợiý?"
...
Tại sao cần cả 3:
- Precision cao, Recall thấp → gợiý chính xác nhưng bỏ sót nhiều
- Recall cao, Precision thấp → không bỏ sót nhưng gợiý太多 "rác"
...
**Trả lời:**
```
Precision@K = (số gợiý đúng) / K
  → User có 2 gợiý đúng trong 6 → Precision = 2/6 = 0.33

Hit Rate = (số user có ≥1 gợiý đúng) / (tổng user)
  → 9/20 user có ít nhất 1 gợiý đúng → Hit Rate = 0.45
```

Hit Rate quan trọng hơn trong thực tế: user chỉ cần **1 gợiý đúng** là hài lòng, không cần tất cả 6 đều đúng.
...
**Trả lời:**
MAP có xét **thứ tự** gợiý:

```
User A: gợiý đúng ở vị trí 1 → AP = 1/1 = 1.0
User B: gợiý đúng ở vị trí 6 → AP = 1/6 = 0.17

Cùng 1 gợiý đúng, nhưng MAP đánh giá User A cao hơn
vì gợiý đúng ngay đầu → user体验 tốt hơn
```
...
2. Ẩn test item, dùng phần còn lại để gợiý
...
1. **Relative comparison vẫn有意义**: Hybrid > Popularity > Random dù dataset nhỏ
2. **Synthetic data**: Có thể tạo 100-500 users để验证 kết quả
3. **Đồ án đại học**: Focus vào thuật toán và implementation, không phải production-grade evaluation
4. **Kết quả reproducible**: Dùng seed cố định (Random(42), Random(20260501L)) → kết quả一致 khi chạy lại
...
2. Kết quả gợiý: @Cacheable("recommendations")
   - Key = userId
...
**Trả lời:**
1. **Hybrid approach** luôn tốt hơn单一 approach trong recommendation
...
4. **Caching là必需**: Ma trận similarity tính慢 → cache 30 phút
