...
## 8. Góc Giảng Viên: Bộ Câu Hỏi Chuyên Sâu Về Hệ Thống Thực Tế (Code-based Q&A)

*Nhóm câu hỏi này tập trung khai thác sâu vào cấu trúc mã nguồn Java thực tế của dự án. Đây là vũ khí cực lớn giúp nhóm chứng minh mình là người thực sự code và nắm rõ từng dòng code trong bộ não `RecommendationService.java`.*

**🛑 Câu 11: Em lưu trữ Ma trận người dùng - sản phẩm (User-Product Matrix) và Ma trận độ tương đồng (Similarity Matrix) ở đâu? Có lưu trong Database không? Tại sao?**
* **👨‍🏫 Ý đồ của giảng viên:** Đánh giá kiến thức thực tế về kiến trúc hệ thống và tối ưu tài nguyên lưu trữ.
* **✅ Trả lời tự tin:**
  > *"Dạ thưa thầy/cô, để tối ưu hóa tốc độ xử lý real-time, hai ma trận này được nhóm em lưu trữ **trực tiếp trong RAM (In-memory)** dưới dạng các `volatile Map` trong `RecommendationService.java`:
  > - `userProductMatrix`: lưu trữ vector mua hàng của từng user.
  > - `userSimilarityMatrix`: lưu trữ điểm tương đồng Cosine giữa các cặp user.
  > 
  > **Lý do không lưu trong Database:** Việc liên tục đọc/ghi và tính toán trên hàng nghìn dòng dữ liệu từ DB cho mỗi lượt F5 trang chủ của khách hàng sẽ gây tắc nghẽn truy vấn (DB Bottleneck). Lưu trên RAM giúp phản hồi gần như lập tức (dưới vài chục mili-giây).
  > 
  > **Cách xử lý khi mất dữ liệu RAM (Ví dụ server bị restart):** Hệ thống của em được thiết kế cơ chế tự phục hồi thông qua hàm `ensureMatricesBuilt()`. Mỗi khi có request gợi ý đầu tiên được gửi tới, hệ thống sẽ tự động quét qua DB (`OrderItemRepository.findAllCompletedOrderItems()`), tái lập toàn bộ ma trận từ lịch sử mua hàng thực tế và lưu vào RAM, đảm bảo an toàn tuyệt đối mà không sợ mất dữ liệu."*

**🛑 Câu 12: Cơ chế Caching gợi ý hoạt động như thế nào trong mã nguồn của em? Khi nào thì danh sách gợi ý bị hủy (invalidated/evicted) để cập nhật mới?**
* **👨‍🏫 Ý đồ của giảng viên:** Đánh giá khả năng tối ưu hóa hiệu năng ứng dụng Spring Boot.
* **✅ Trả lời tự tin:**
  > *"Dạ, nhóm em sử dụng cơ chế Caching mạnh mẽ của **Spring Boot** thông qua annotation `@Cacheable(value = "recommendations", key = "#userId")` đặt ở hàm `getRecommendations(UUID userId)`.
  > - **Quy trình hoạt động:** Khi khách hàng truy cập Trang chủ, hệ thống sẽ kiểm tra xem danh sách gợi ý của họ đã có trong cache chưa. Nếu có, nó sẽ trả về ngay lập tức (chỉ mất dưới 10ms). Nếu chưa, hệ thống mới bắt đầu tính toán thuật toán Hybrid và lưu vào Cache.
  > - **Thời gian sống của ma trận (TTL):** Ma trận thô được cache 30 phút (`MATRIX_TTL_MS = 30 * 60 * 1000`).
  > - **Cơ chế Hủy Cache (Eviction):** Nhóm em áp dụng phương thức **Event-driven Eviction** (Xóa cache dựa trên sự kiện). Cụ thể, khi một khách hàng đặt hàng thành công, hệ thống sẽ gọi hàm `evictCacheForUser(userId)` có gắn `@CacheEvict` để xóa ngay lập tức kết quả cache gợi ý của user đó. Đồng thời ma trận toàn hệ thống cũng được kích hoạt xây dựng lại bằng việc đặt `matrixLastBuilt = 0` nhằm đảm bảo đơn hàng mới nhất đã được học vào ma trận trong lần truy vấn tiếp theo."*

**🛑 Câu 13: Trong hàm `ensureMatricesBuilt()`, em làm thế nào để chuẩn hóa (normalize) dữ liệu mua hàng của user? Tại sao việc chuẩn hóa này lại cực kỳ quan trọng đối với thuật toán?**
* **👨‍🏫 Ý đồ của giảng viên:** Kiểm tra kỹ năng xử lý tiền dữ liệu (Data Preprocessing) trong AI.
* **✅ Trả lời tự tin:**
  > *"Dạ, trong cơ sở dữ liệu, hành vi mua sắm của khách hàng được ghi nhận dưới dạng **Số lượng sản phẩm** mua thành công. Có những khách hàng mua rất nhiều (ví dụ mua 10 cốc cà phê cho công ty) và có khách chỉ mua 1-2 cốc.
  > - Nếu đưa trực tiếp số cốc này vào tính toán, các vector của khách hàng mua số lượng lớn sẽ áp đảo hoàn toàn, làm sai lệch phép tính Cosine.
  > - **Giải pháp của nhóm:** Trong hàm `ensureMatricesBuilt()`, nhóm em đã tiến hành **chuẩn hóa Max-Normalization (tỷ lệ 0 đến 1)** cho vector của mỗi user. Em tìm ra số lượng cốc lớn nhất mà user đó từng mua cho 1 sản phẩm cụ thể (`maxVal`), sau đó chia tất cả các lượng mua của sản phẩm khác cho `maxVal` đó:
  >   `userVec.replaceAll((k, v) -> v / maxVal);`
  > - Phép chuẩn hóa này giúp đưa tất cả hành vi mua sắm về cùng một hệ quy chiếu dạng tỷ lệ phần trăm sở thích (từ 0.0 đến 1.0), giúp phép đo tương đồng phản ánh chính xác 'gu thưởng thức' của người dùng."*

**🛑 Câu 14: Thuật toán gợi ý Cross-selling (Bán chéo trong giỏ hàng) của em xử lý thế nào khi một sản phẩm mới tinh chưa từng có ai mua chung với món khác?**
* **👨‍🏫 Ý đồ của giảng viên:** Đánh giá khả năng viết mã nguồn an toàn (Robust Code) và tư duy phòng vệ (Defensive Programming).
* **✅ Trả lời tự tin:**
  > *"Dạ thưa thầy/cô, đây là tình huống kinh điển khi triển khai hệ thống thực tế. Hàm `getCrossSellingRecommendations(UUID productId)` của nhóm em được thiết kế với cấu trúc **3 tầng phòng vệ (Multi-tier Fallback)** cực kỳ vững chắc:
  > - **Tầng 1 (Co-occurrence):** Tìm các đơn hàng lịch sử có chứa sản phẩm này, đếm tần suất mua kèm với các món nước khác để gợi ý Top 3 món.
  > - **Tầng 2 (Category-based Fallback):** Nếu sản phẩm mới tinh chưa có quá 2 người mua, hệ thống sẽ tự động gọi hàm `getFallbackCrossSelling()`. Hàm này sẽ lọc ra các sản phẩm **cùng danh mục (Category)** với sản phẩm hiện tại trong menu, trộn ngẫu nhiên (`Collections.shuffle`) và gợi ý 3 món khác cùng danh mục.
  > - **Tầng 3 (Best Sellers Fallback):** Nếu sản phẩm không thuộc danh mục nào hoặc danh mục bị thiếu đồ uống sẵn có, hệ thống sẽ fallback tiếp về danh sách **Best Sellers (Bán chạy nhất hệ thống)** để điền đầy 3 gợi ý. Nhờ vậy hệ thống luôn chạy trơn tru mà không bao giờ trả về danh sách trống hay gây lỗi giao diện giỏ hàng."*

**🛑 Câu 15: Nếu quán cà phê của chúng ta có một món đồ uống bị hết hàng (hoặc ngừng bán), hệ thống AI của em có tiếp tục gợi ý món đó nữa hay không? Em xử lý điều này như thế nào trong code?**
* **👨‍🏫 Ý đồ của giảng viên:** Đánh giá khả năng kết nối logic AI với logic kinh doanh thực tế (Business Logic Integration).
* **✅ Trả lời tự tin:**
  > *"Dạ thưa thầy/cô, đây là sự khác biệt giữa thuật toán AI lý thuyết và hệ thống AI thực tế. Nếu gợi ý một món nước đã hết hàng cho khách sẽ gây trải nghiệm cực kỳ tệ và tổn thất doanh thu.
  > - Trong mã nguồn Java của em, tại tất cả các hàm gợi ý như `getRecommendations()` (Dòng 130), `getBestSellers()` (Dòng 138-155), `getCrossSellingRecommendations()` (Dòng 224), sau khi thuật toán AI tính toán và tìm ra Top ID sản phẩm tốt nhất, hệ thống luôn thực hiện một bộ lọc kiểm tra trạng thái khả dụng của sản phẩm thông qua:
  >   `.filter(Product::isAvailable)`
  > - Trạng thái `isAvailable` (còn hàng/đang bán) được quản lý trực tiếp bởi Admin của quán. Nếu Admin chuyển trạng thái sản phẩm sang 'Ngừng bán' hoặc 'Hết hàng', hệ thống gợi ý sẽ lập tức loại bỏ sản phẩm đó khỏi danh sách đề xuất trong vòng chưa tới 1 mili-giây, đảm bảo khách hàng chỉ nhìn thấy những món nước thực sự có thể đặt hàng được ngay."*

**🛑 Câu 16: Sự khác biệt lớn nhất giữa "Hệ thống gợi ý" (Recommendation System) và "Hệ thống tìm kiếm" (Search System) trong một ứng dụng là gì? Tại sao chúng ta cần cả hai?**
* **👨‍🏫 Ý đồ của giảng viên:** Đo lường sự thấu hiểu về bản chất trải nghiệm người dùng thông minh trong TMĐT.
* **✅ Trả lời tự tin:**
  > *"Dạ thưa thầy/cô, sự khác biệt cốt lõi nằm ở chủ thể chủ động và cơ chế tương tác:*
  > - **Hệ thống tìm kiếm (Search System):** Hoạt động theo cơ chế **Kéo (Pull)**. Khách hàng là người chủ động, họ đã biết rõ mình muốn gì (ví dụ: gõ từ khóa 'Trà đào') và hệ thống có nhiệm vụ trả về kết quả khớp nhất.
  > - **Hệ thống gợi ý (Recommendation System):** Hoạt động theo cơ chế **Đẩy (Push)**. Khách hàng là người thụ động, họ chưa chắc đã biết mình muốn uống gì. AI sẽ phân tích hành vi lịch sử của họ và của cộng đồng để chủ động đề xuất những sản phẩm phù hợp nhất lên màn hình.
  > 
  > Chúng ta cần cả hai vì chúng tương trợ lẫn nhau: Tìm kiếm giúp giải quyết các nhu cầu mua sắm có chủ đích tức thì, trong khi Gợi ý giúp khai phá nhu cầu tiềm ẩn của khách hàng, tăng tỷ lệ giữ chân (Retention Rate) và kích thích doanh số bán kèm (Cross-selling) hiệu quả."*

**🛑 Câu 17: Làm sao em đánh giá được hiệu quả kinh doanh thực tế của Hệ thống gợi ý AI này khi đưa vào vận hành thương mại? Có những chỉ số thực tế nào ngoài Precision và Recall?**
* **👨‍🏫 Ý đồ của giảng viên:** Đánh giá khả năng liên kết toán học AI với giá trị thực tiễn doanh nghiệp (Business value metrics).
* **✅ Trả lời tự tin:**
  > *"Dạ thưa thầy/cô, trong thực tế kinh doanh, ngoài các chỉ số kỹ thuật như Precision hay Recall, người ta sẽ dùng phương pháp **A/B Testing** (chia người dùng thành 2 nhóm: nhóm A dùng gợi ý AI, nhóm B gợi ý ngẫu nhiên hoặc không gợi ý) để đo lường các chỉ số kinh doanh cốt lõi sau:*
  > 1. **CTR (Click-Through Rate - Tỷ lệ nhấp):** Tỷ lệ phần trăm khách hàng click vào các sản phẩm được AI đề xuất trên màn hình. CTR càng cao chứng tỏ tiêu đề/hình ảnh và vị trí gợi ý càng hấp dẫn.
  > 2. **CVR (Conversion Rate - Tỷ lệ chuyển đổi đơn hàng):** Tỷ lệ phần trăm khách hàng thực sự bấm thanh toán sản phẩm sau khi click xem gợi ý.
  > 3. **AOV (Average Order Value - Giá trị đơn hàng trung bình):** Đo lường xem doanh thu trung bình trên mỗi đơn hàng có tăng lên nhờ các món gợi ý bán chéo (Cross-selling) trong giỏ hàng hay không.
  > 4. **Doanh thu đóng góp từ AI (AI-attributed Revenue):** Đo lường xem có bao nhiêu phần trăm tổng doanh thu của quán đến trực tiếp từ các sản phẩm được khách đặt thông qua việc bấm vào danh mục gợi ý."*

**🛑 Câu 18: Trong thuật toán KNN để tính điểm gợi ý lọc cộng tác (`computeCFScores`), tại sao em lại chia tổng điểm dự đoán cho tổng giá trị tuyệt đối độ tương đồng `similaritySum`? Công thức này có ý nghĩa gì?**
* **👨‍🏫 Ý đồ của giảng viên:** Kiểm tra sâu kiến thức toán học về công thức **Trung bình có trọng số (Weighted Average)** của thuật toán KNN.
* **✅ Trả lời tự tin:**
  > *"Dạ thưa thầy/cô, đây là công thức tính **Trung bình có trọng số (Weighted Average)** kinh điển trong Hệ gợi ý:*
  > 
  > $$\text{Predicted Score}(u, p) = \frac{\sum_{n \in K} \text{Similarity}(u, n) \times \text{Rating}(n, p)}{\sum_{n \in K} |\text{Similarity}(u, n)|}$$
  > 
  > *Ý nghĩa của việc chia cho tổng độ tương đồng `similaritySum`:*
  > - Nếu chúng ta chỉ cộng dồn điểm của các láng giềng (`similarity * neighborRating`), thì điểm số dự đoán của một sản phẩm sẽ bị thổi phồng cực kỳ lớn khi số lượng láng giềng $K$ tăng lên hoặc khi các láng giềng có điểm tương đồng cao.
  > - Phép chia này đóng vai trò **chuẩn hóa (normalization)**, giúp đưa điểm số dự đoán cuối cùng về đúng thang đo gốc của dữ liệu (tương ứng khoảng `0.0 - 1.0`), giúp việc so sánh xếp hạng (ranking) giữa các sản phẩm diễn ra công bằng và chính xác."*

**🛑 Câu 19: Trong code của em, nếu 2 người dùng hoàn toàn không có sản phẩm chung nào từng mua thì độ tương đồng Cosine Similarity giữa họ sẽ là bao nhiêu? Code của em xử lý ngoại lệ này như thế nào để tránh lỗi chia cho 0 (Division by Zero)?**
* **👨‍🏫 Ý đồ của giảng viên:** Đánh giá khả năng lập trình thuật toán toán học an toàn (Robust Algorithm Implementation) của sinh viên CNTT.
* **✅ Trả lời tự tin:**
  > *"Dạ thưa thầy/cô, nếu 2 người dùng hoàn toàn không mua chung bất kỳ món nước nào, phần giao của 2 vector mua hàng của họ sẽ là tập hợp rỗng (`commonKeys.isEmpty()`).*
  > * Trong hàm `cosineSimilarity()` (Dòng 357 của [RecommendationService.java](file:///d:/Project/BTL-TTNT/src/main/java/com/coffeeshop/service/RecommendationService.java)), nhóm em đã chủ động viết mã nguồn phòng vệ ở ngay đầu hàm:*
  >   `if (commonKeys.isEmpty()) { return 0.0; }`
  > * Hệ thống sẽ lập tức trả về độ tương đồng bằng `0.0` (không tương đồng) mà không cần tính toán tiếp. Đồng thời, ở cuối hàm, trước khi thực hiện phép chia lấy Cosine, em cũng kiểm tra độ dài (magnitude) của 2 vector:*
  >   `if (mag1 == 0.0 || mag2 == 0.0) { return 0.0; }`
  > * Việc này giúp **chặn đứng hoàn toàn lỗi Division by Zero (chia cho 0)** trong toán học, đảm bảo máy chủ Spring Boot chạy cực kỳ ổn định, không bao giờ bị crash do lỗi tính toán ma trận."*

**🛑 Câu 20: Hệ gợi ý của em có thể gặp phải hiện tượng "Bong bóng bộ lọc" (Filter Bubble) hay không? Khái niệm này là gì và nhóm đã giải quyết ra sao?**
* **👨‍🏫 Ý đồ của giảng viên:** Khảo sát hiểu biết về giới hạn và thiên kiến của AI (AI bias & limitations).
* **✅ Trả lời tự tin:**
  > *"Dạ thưa thầy/cô, **Filter Bubble (Bong bóng bộ lọc)** là hiện tượng AI chỉ gợi ý đi gợi ý lại những gì người dùng đã thích hoặc đã mua trong quá khứ, vô tình cô lập họ trong một 'bong bóng sở thích' và tước đi cơ hội tiếp cận các sản phẩm mới lạ.*
  > * Đối với quán cafe, nếu khách suốt ngày nhận gợi ý 'Cà phê đen' thì hệ gợi ý sẽ bị nhàm chán và giảm hiệu quả kinh doanh món mới.
  > * **Nhóm em đã giải quyết việc này bằng hai cách cực kỳ khoa học:**
  >   1. **Trọng số Collaborative Filtering cao (60%):** Thuật toán Lọc cộng tác tìm kiếm những người có gu tương tự để gợi ý những món **họ chưa từng mua** nhưng những láng giềng đã mua và yêu thích. Việc này giúp mở rộng ranh giới sở thích của khách hàng.
  >   2. **Trộn ngẫu nhiên thông minh (Stochastic Fallback):** Trong hàm gợi ý bán chéo (Cross-selling), khi rơi vào fallback cùng danh mục, em sử dụng `Collections.shuffle(categoryProducts)` để xáo trộn ngẫu nhiên danh sách trước khi lấy ra 3 món. Sự ngẫu nhiên thông minh này mang lại yếu tố khám phá thú vị, giúp phá vỡ bong bóng bộ lọc và kích thích khách hàng thử nghiệm nhiều món nước mới của quán."*
